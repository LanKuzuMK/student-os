...
<div class="modal-overlay" id="roleModal" role="dialog" aria-modal="true" aria-labelledby="roleModalTitle"><div class="modal"><h3 id="roleModalTitle">Change role</h3><form method="post" action="/admin/users/role"><input type="hidden" name="userId" id="roleUserId"><input type="hidden" name="csrfToken" value="<%=csrfToken%>"><label class="visually-hidden" for="newRole">New role</label><select id="newRole" name="newRole" class="form-control" style="margin-bottom:14px"><option value="STUDENT">STUDENT</option><option value="MODERATOR">MODERATOR</option><option value="ADMIN">ADMIN</option></select><div class="modal-actions"><button type="button" class="btn btn-secondary" onclick="closeModals()">Cancel</button><button type="submit" class="btn btn-primary">Save role</button></div></form></div></div>
<div class="modal-overlay" id="resetModal" role="dialog" aria-modal="true" aria-labelledby="resetModalTitle"><div class="modal"><h3 id="resetModalTitle">Reset password for <span id="resetEmail"></span></h3><form method="post" action="/admin/users/reset-password"><input type="hidden" name="userId" id="resetUserId"><input type="hidden" name="csrfToken" value="<%=csrfToken%>"><label class="visually-hidden" for="resetPassword">New password</label><input id="resetPassword" class="form-control" type="password" name="newPassword" placeholder="8+ characters, with a letter and number" required minlength="8" pattern="(?=.*[A-Za-z])(?=.*\d)\S{8,128}" title="Use 8 to 128 characters with a letter, a number, and no spaces." style="margin-bottom:14px"><div class="modal-actions"><button type="button" class="btn btn-secondary" onclick="closeModals()">Cancel</button><button type="submit" class="btn btn-primary">Reset password</button></div></form></div></div>
<script>
let lastModalTrigger = null;
function openModal(modalId, focusSelector) { lastModalTrigger = document.activeElement; const modal = document.getElementById(modalId); modal.classList.add('open'); window.setTimeout(() => modal.querySelector(focusSelector).focus(), 0); }
function openRoleModal(id, role) { document.getElementById('roleUserId').value = id; document.querySelector('#roleModal select').value = role; openModal('roleModal', '#newRole'); }
function openResetModal(id, email) { document.getElementById('resetUserId').value = id; document.getElementById('resetEmail').textContent = email; openModal('resetModal', '#resetPassword'); }
function closeModals() { document.querySelectorAll('.modal-overlay').forEach(m => m.classList.remove('open')); if (lastModalTrigger) lastModalTrigger.focus(); }
document.querySelectorAll('.modal-overlay').forEach(m => m.addEventListener('click', e => { if (e.target === m) closeModals(); }));
document.addEventListener('keydown', e => { if (e.key === 'Escape' && document.querySelector('.modal-overlay.open')) closeModals(); });
</script>
