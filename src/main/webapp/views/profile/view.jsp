...
<div class="app-container">
    ...
    <main class="main-content">
        ...
        <section class="public-profile-hero">
            <div class="public-profile-avatar"><div class="profile-photo-placeholder profile-photo-xl">${profile.displayName.substring(0,1).toUpperCase()}</div></div><div class="public-profile-intro"><p class="eyebrow">StudentOS member</p><h1><c:out value="${profile.displayName}"/></h1><c:if test="${not empty profile.major}"><p class="profile-major"><c:out value="${profile.major}"/><c:if test="${not empty profile.university}"> · <c:out value="${profile.university}"/></c:if></p></c:if><p class="profile-email"><c:out value="${profile.email}"/></p></div>
            <div class="public-profile-actions"><c:choose><c:when test="${isOwnProfile}"><a class="btn btn-primary" href="/profile">Edit profile</a></c:when><c:otherwise><a class="btn btn-primary" href="/collaborations?to=<c:out value='${profile.userId}'/>">Propose collaboration</a><a class="btn btn-secondary" href="/messages?toEmail=<c:out value='${profile.email}'/>">Message student</a><form method="post" action="/saved/add"><input type="hidden" name="csrfToken" value="<c:out value='${csrfToken}'/>"><input type="hidden" name="targetType" value="PROFILE"><input type="hidden" name="targetId" value="<c:out value='${profile.userId}'/>"><input type="hidden" name="returnTo" value="/profile/view?id=<c:out value='${profile.userId}'/>"><button class="text-action" type="submit">Save profile</button></form></c:otherwise></c:choose></div>
        </section>
